...
We use a privately hosted [Hashicorp's Vault](https://www.vaultproject.io/) instance for key management and encryption-as-a-service. 
Every new organisation on envsecrets gets it's own `aes256-gcm96` encryption key. 
Which is an AES-256 wrapped symmetric key with GCM using a 96-bit nonce size AEAD.

All secrets inside that organisation, be it in any project or environment, are encrypted with the same key.

As a good engineering practice, we store the encryption keys inside Vault but the encrypted secret values in a separate postgres database.
This also makes it easier to query the database by specific secret names/keys.

## Access Protection

We implemented very specific row-level and column-level security on our transactional database to ensure only the data and relationships belonging to you are accessible by you.
And no one else can gain visibility into your rows.
...
1. Leverage Vault's inbuilt key-rotation.
1. We don't have to take the risk of implementing encryptions from scratch and get sleep-less nights.
